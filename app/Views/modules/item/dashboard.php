				...
				<table id="mlsTable">
					...
					<tbody>
						<?php foreach ($list as $mls_list): ?> <!-- Sample Data Rows -->
							<?php $id = !empty($mls_list['item_id']) ? $mls_list['item_id'] : ""; ?>
							<tr id="row_<?php echo $id; ?>">
								<td><?= !empty($mls_list['item_name']) ? $mls_list['item_name'] : ""; ?></td>
								<td><?= !empty($mls_list['item_HSN_code']) ? $mls_list['item_HSN_code'] : ""; ?></td>
                                                                <td><?= !empty($mls_list['price']) ? $mls_list['price'] : ""; ?></td>
                                                                <td><?= !empty($mls_list['item_purchase_date']) ? date("d/m/Y   ", strtotime($mls_list['item_purchase_date'])) : ""; ?></td>
                                                                <td><?= !empty($mls_list['quantity']) ? $mls_list['quantity'] : ""; ?></td>
								<td>
									<a href="<?php echo base_url() . 'edit-list/' . base64_encode($id); ?>"
